...
def compute_average_triplet_loss(
    ...
    # Flatten the (B, S, ...) sample lists to (B*S, ...) for a single
    # encoder pass, then split back into B chunks of size S.
    pos_list = pos_list.reshape((-1, *pos_list.shape[2:])).to(device)
    neg_list = neg_list.reshape((-1, *neg_list.shape[2:])).to(device)

    pos_encodes = model.encoder(pos_list).split(num_samples, dim=0)
    ...
    # Broadcast anchor_encode (B, D) to (S, B, D) using expand instead
    # of stack so we don't allocate S copies of the tensor.
    anchor_encode = anchor_encode.unsqueeze(0).expand(num_samples, -1, -1)
...
def compute_loss(
    ...
    triplet_loss_transcriptomics = (
        compute_average_triplet_loss(
            model, anchor_encode,
            pos_transcriptomics_list, neg_transcriptomics_list,
            triplet_loss, device,
        )
        if transcriptomics_weight > 0 and triplet_alpha
        else zero
    )
    triplet_loss_morphology = (
        compute_average_triplet_loss(
            model, anchor_encode,
            pos_morphology_list, neg_morphology_list,
            triplet_loss, device,
        )
        if morphology_weight > 0 and triplet_alpha
        else zero
    )
    rec_loss_transcriptomics = (
        compute_reconstruction_loss(
            anchor[:, :obsm_transcriptomics_dim],
            anchor_decode[:, :obsm_transcriptomics_dim],
            rec_loss,
        )
        if transcriptomics_weight > 0 and rec_alpha
        else zero
    )
    rec_loss_morphology = (
        compute_reconstruction_loss(
        ...
        if morphology_weight > 0 and rec_alpha
        else zero
    )

    total_loss = (
        transcriptomics_weight * (rec_alpha * rec_loss_transcriptomics + triplet_alpha * triplet_loss_transcriptomics)
